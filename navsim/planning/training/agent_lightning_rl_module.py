...
from navsim.agents.dp_rl.dp_rl_agent import DPRLAgent
from navsim.common.dataclasses import Trajectory, SensorConfig
...
# The reason of modifying the training pipeline is due to the availability of the DP RL agent.
def evaluate_pdms_batch(
    token_list: List[str],
    agent,
    ...
    """
    批量评估一批 token（多个场景）的 PDMS（Stage1）。
    返回 pandas.DataFrame，每行包含 token, pdms score 和各子指标。
    """
    simulator: PDMSimulator = instantiate(cfg.simulator)
    # sim = PDMSimulator(**simulator_cfg)
    scorer: PDMScorer = instantiate(cfg.scorer)
    # scorer = PDMScorer(**scorer_cfg)
    ...
    # 提取 final PDMS 和各子指标
    # double check the contents of the row.
    out = df[[
        "token",
        "pdm_score",
...
class AgentLightningRLModule(pl.LightningModule):
    """Pytorch lightning wrapper for learnable agent."""

    def __init__(self,
                 agent: AbstractAgent,
                 cfg: DictConfig, # this config needs to be very complete.
                 combined: bool = False,
                 ):
        """
    ...
    def _step(self, batch: Tuple[Dict[str, Tensor], Dict[str, Tensor]], logging_prefix: str) -> Tensor:
        ...
        pdm_scores = evaluate_pdms_batch(tokens, self.agent, self.metric_cache_loader,
                                         self.scene_loader, self.cfg)
        pdm_scores = pdm_scores.set_index('token').to_dict(orient='index')

        if isinstance(self.agent, TransfuserAgent):
            loss, loss_dict = self.agent.compute_loss(features, targets, prediction)
        elif isinstance(self.agent, DPRLAgent):
            # the loss will be computed based on each group.
            loss, loss_dict = self.agent.compute_loss(features, targets, prediction, pdm_scores)
        else:
